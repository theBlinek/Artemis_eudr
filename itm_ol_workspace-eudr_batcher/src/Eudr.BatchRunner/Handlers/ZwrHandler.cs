...
/// <summary>
/// Handles type 200 FORWARD — return from production (ZW). (§9.3)
/// Inserts ZWR with NEGATIVE RAW_MATERIAL_ILOSC (returned quantity).
/// Credits the matching RPR's ILOSC_ROZ so the stock is available for FIFO again.
/// </summary>
public sealed class ZwrHandler(IEudrRepository repo, ILogger<ZwrHandler> logger) : IHandler
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (positions.Count == 0)
        {
            logger.LogInformation(
                "ZWR: no EUDR-tagged positions for MAG_DOK_NAG_ID={Doc} — skipping", evt.MagDokNagId);
        ...
        foreach (var pos in positions)
        {
            ...
            if (sourceRpr is null)
            {
                logger.LogWarning(
                    "ZWR: no RPR found for kod={Kod} seria={Seria} (doc {Doc}) — inserting ZWR without DDS",
                    pos.ZamiennikKod, evt.SeriaId, evt.MagDokNagId);
            }
            ...
            await repo.InsertLedgerRowAsync(new LedgerRow
            {
                ...
                RawMaterialIlosc = -pos.Ilosc,    // ZWR is always negative
